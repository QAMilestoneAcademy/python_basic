...
class ServiceTaxCalculator():
    def __init__(self):
        window = Tk()
        window.title("Service Tax Calculator App")
        ...
        tip_percents = Label(window,text ="Tax Percentages",bg="blue",fg="white")
        tip_percents.grid(column=0,row=0,padx=15,pady=10)

        bill_amount = Label(window,text ="Bill Amount",bg="blue",fg="white")
        bill_amount.grid(column=1,row=0,padx=15,pady=10)

        bill_amount_entry = Entry(window,textvariable =self.bill_amount,width =14)
        ...
        bill_total_lbl = Label(window, text="Bill Total", bg="green", fg="white")
        ...
        calculate_btn = Button(window, text="Calculate", bg="black", fg="white", command=self.calculate)
        calculate_btn.grid(column=1, row=7, padx=15)

        clear_btn = Button(window, text="Clear", bg="dark grey", fg="white", command=self.clear)
        clear_btn.grid(column=2, row=7)

        window.mainloop()

    def calculate(self):
        pre_tip = float(self.bill_amount.get())
